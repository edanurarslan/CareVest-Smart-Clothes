...
class HomeScreenState extends State<HomeScreen> {
  ...
  // 📌 Firebase'den sağlık verilerini çekme
  Future<void> _fetchHealthData() async {
    String? userId = _auth.currentUser?.uid;
    if (userId == null) return;
    ...
    try {
      _firestore.collection('health_data').doc(userId).snapshots().listen((snapshot) {
        if (snapshot.exists && mounted) { // 📌 mounted kontrolü eklendi
          setState(() {
            healthData = snapshot.data();
          });
        }
      ...
      if (mounted) { // 📌 Widget kapanmışsa setState çağrılmasın
        print("🔥 Firebase Hatası: $e");
      }
    }
  }

  // 📌 Kullanıcı çıkış yaparken onay ekranı
  Future<bool> _showLogoutDialog(BuildContext context) async {
    return await showDialog(
          context: context,
          builder: (context) => AlertDialog(
